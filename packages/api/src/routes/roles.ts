...
const roles = new Hono<{ Bindings: Env }>()
  ...
  .get('/:id', async (c) => {
    const roleId = parseId(c.req.param('id'), 'id');
  ...
  .post('/:id/permissions', validate('json', z.object({ permissionIds: z.array(z.string()) })), async (c) => {
    ...
    const parsedPermissionIds = permissionIds.map((permissionId) => parseId(permissionId, 'permissionId'));
    const result = await assignPermissionsToRole(c.env.DB, roleId, parsedPermissionIds);
    await bumpAuthzCacheVersion(c.env.PERMISSION_CACHE);
    return c.json(success(result));
  });
