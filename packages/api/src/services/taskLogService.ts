...
export const findLogs = async (d1: D1Database, pagination: PaginationQuery) => {
  const db = getDb(d1);
  const query = db.select().from(taskLogs).orderBy(desc(taskLogs.executedAt));
  const countSql = sql`SELECT count(*) as count FROM ${taskLogs}`;
  return paginate(db, query, countSql, pagination);
};

export const deleteLog = async (d1: D1Database, id: number) => {
  const db = getDb(d1);
  const result = await db.delete(taskLogs).where(eq(taskLogs.id, id)).returning();
  if (result.length === 0) {
