...
export const findDailyAttendanceLogs = async (d1: D1Database, pagination: PaginationQuery) => {
  const db = getDb(d1);
  const query = db.select().from(dailyAttendanceLogs).orderBy(desc(dailyAttendanceLogs.createdAt));
  const countSql = sql`SELECT count(*) as count FROM ${dailyAttendanceLogs}`;
